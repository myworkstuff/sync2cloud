...
const copyData = async () => {
  ...
  try {    
    ...
    const [results] = await sourceConnection.query(`SELECT * FROM frontend.pospayment WHERE bizdate  = '2024-07-03'`);    
    const insertPromises = results.map(row => {
      const columns = Object.keys(row).join(',');
      const values = Object.values(row).map(value => `'${detectDateType(value)}'`).join(',');
      return destinationConnection.query(`INSERT INTO cksgroup_intra.pospayment (${columns}) VALUES (${values})`);
    });    
...
console.clear();
console.log('Starting copying pospayment');
copyData();
