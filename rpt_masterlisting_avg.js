...
async function process_data(concept,concept_){    
    ...
    try {
        ...
        AND LEFT(a.created_at,7)>=LEFT(DATE(DATE_SUB(NOW(), INTERVAL 12 MONTH)),7)
        GROUP BY a.itemcode
        `;
...
ROUND(SUM(CASE WHEN a.SoldWeight >= FLOOR(b.avgqty) THEN a.SoldWeight ELSE 0 END), 2) AS TotalSoldWeight,
ROUND(SUM(CASE WHEN a.SoldWeight >= FLOOR(b.avgqty) THEN a.SoldAmt ELSE 0 END), 2) AS TotalSoldAmt,
ROUND(SUM(CASE WHEN a.SoldWeight >= FLOOR(b.avgqty) THEN a.LastCost ELSE 0 END), 2) AS LastCost,
COUNT(DISTINCT CASE WHEN a.SoldWeight >= FLOOR(b.avgqty) THEN a.location END) AS 'No_Involved_outlet',
COALESCE(ROUND((ROUND(SUM(CASE WHEN a.SoldWeight >= FLOOR(b.avgqty) THEN a.SoldAmt ELSE 0 END), 2)
- ROUND(SUM(CASE WHEN a.SoldWeight >= FLOOR(b.avgqty) THEN a.LastCost ELSE 0 END), 2)) 
/ COUNT(DISTINCT CASE WHEN a.SoldWeight >= FLOOR(b.avgqty) THEN a.location END), 2),0) AS Profit_Outlet,
ROUND((ROUND(SUM(CASE WHEN a.SoldWeight >= FLOOR(b.avgqty) THEN a.SoldAmt ELSE 0 END), 2)
- ROUND(SUM(CASE WHEN a.SoldWeight >= FLOOR(b.avgqty) THEN a.LastCost ELSE 0 END), 2)), 2) AS Profit,
...
LEFT JOIN (
	SELECT itemcode, ROUND(IF(SoldbyWeight = 1, SUM(weightvalue), SUM(qty)) / COUNT(DISTINCT location), 2) AS avgqty, 
	   COUNT(DISTINCT location) AS total_outlet
	FROM frontend.poschild	
	WHERE LEFT(bizdate, 7)>=LEFT(DATE(DATE_SUB(NOW(), INTERVAL 12 MONTH)),7)
	AND itemcode ='${_itemcode}'
	AND LEFT(location,1) ${concept_}
...
GROUP BY a.itemcode, FLOOR(b.avgqty)`;                
            ...
            const values = Object.values(row).map(value => `'${detectDateType(value)}'`).join(',');
                return connWeb.query(`REPLACE INTO cksgroup_intra.tbl_masterlisting_avg (${columns}) VALUES (${values})`);
            }); 
...
async function process_sales(){
    console.clear();
    await process_data('sh',"<> 'f'");console.log('Populating SH');  
