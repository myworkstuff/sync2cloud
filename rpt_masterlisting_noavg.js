...
async function process_data(concept,concept_){    
    ...
    try {
        ...
        AND LEFT(a.created_at,7)>='2024-01-01' 
        GROUP BY a.itemcode
        `;
    ...
    ROUND((ROUND(SUM(a.SoldAmt), 2)- ROUND(SUM(a.LastCost), 2)) / COUNT(DISTINCT a.location), 2) AS Profit_Outlet,
    ...
    FROM (
        ...
        WHERE LEFT(bizdate, 7)>='2024-01-01'
    ...
    LEFT JOIN (SELECT itemcode,code,name FROM itemmastersupcode GROUP BY itemcode) i ON i.code=hh.code 
    GROUP BY a.itemcode`;                
            ...
            const values = Object.values(row).map(value => `'${detectDateType(value)}'`).join(',');
                return connWeb.query(`REPLACE INTO cksgroup_intra.tbl_masterlisting_noavg (${columns}) VALUES (${values})`);
            }); 
