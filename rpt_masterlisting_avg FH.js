...
async function process_data(concept,concept_){    
    ...
    try {
        ...
        const values = Object.values(row).map(value => `'${detectDateType(value)}'`).join(',');
            return connWeb.query(`insert ignore INTO cksgroup_intra.tbl_masterlisting_avg_FH (${columns}) VALUES (${values})`);
        }); 
...
// process.exit();
            ...
            const insertPromises = tbl_sales.map(row => {
                const columns = Object.keys(row).join(',');
                const values = Object.values(row).map(value => `'${detectDateType(value)}'`).join(',');
                    return connWeb.query(`replace INTO cksgroup_intra.tbl_masterlisting_avg_FH (${columns}) VALUES (${values})`);
            }); 
            await Promise.all(insertPromises)                
            }                                      
        }
