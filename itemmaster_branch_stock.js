...
async function source_to_destination(fields,table_name,LastSync,database_name,tbl_key,syncfield,connPanda,connWeb){
  try {
    let lastsync=LastSync
    let qry="select "+fields+" from "+database_name+"."+table_name+" where "+syncfield+">'"+LastSync+"' limit 250";    
    let tbldata = await connPanda.query(qry); 
    if(await tbldata[0].length>0){
        ...
        tmpDat=tmpDat.slice(0, -1);tmpFld=tmpFld.slice(0, -1);
        qry ="replace into cksgroup_intra."+table_name+"("+tmpFld+")value("+tmpDat+")"      
...
async function get_lastsync(){
    ...
    try {
        ...
        let qry="SELECT *,CAST(last_sync AS CHAR)LastSync FROM db_sync WHERE ACTIVE=1 and ids='6'";
        const lastsync = await connWeb.query(qry); 
        if(await lastsync[0].length>0){
          for (let i = 0; i < lastsync[0].length; i++) {            
            const database_name =lastsync[0][i]['database_name'], table_name =lastsync[0][i]['table_name'], LastSync = lastsync[0][i]['LastSync'];const syncfield =lastsync[0][i]['syncfield'], tbl_key =lastsync[0][i]['tbl_key'];
            const fields= await get_table_schema(table_name,database_name,connPanda);            
            const uLastSync= await source_to_destination(fields,table_name,LastSync,database_name,tbl_key,syncfield,connPanda,connWeb)
