...
const copyData = async () => {
  ...
  try {    
    ...
    const [results] = await sourceConnection.query(`SELECT * FROM frontend.posmain WHERE bizdate = '2024-07-03'`);  
    const insertPromises = results.map(row => {
      ...
      return destinationConnection.query(`INSERT INTO dev_intra.posmain (${columns}) VALUES (${values})`);
    });    
...
console.clear();
console.log('Starting copying POSMAIN');
copyData();
